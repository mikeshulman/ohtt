...
postulate
  ap, : {Δ : Tel} (A B : Δ →Type) (δ : el (ID Δ)) (f : (x : el Δ) → A ⊘ x) (g : (x : el Δ) → B ⊘ x) →
    ap (Λ w ⇒ A ⊘ w × B ⊘ w) (λ x → (f x , g x)) δ ≡ (ap A f δ , ap B g δ)
  ap-fst : {Δ : Tel} (A B : Δ →Type) (δ : el (ID Δ)) (f : (x : el Δ) → A ⊘ x × B ⊘ x) →
    ap A (λ x → fst (f x)) δ ≡ fst (ap (Λ w ⇒ A ⊘ w × B ⊘ w) f δ)
  ap-snd : {Δ : Tel} (A B : Δ →Type) (δ : el (ID Δ)) (f : (x : el Δ) → A ⊘ x × B ⊘ x) →
    ap B (λ x → snd (f x)) δ ≡ snd (ap (Λ w ⇒ A ⊘ w × B ⊘ w) f δ)
