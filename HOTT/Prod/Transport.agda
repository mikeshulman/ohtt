...
postulate
  tr→× : {Δ : Tel} (A B : el Δ → Type) (δ : el (ID Δ)) (u₀ : A (δ ₀) × B (δ ₀)) →
    tr→ (Λ w ⇨ A w × B w) δ u₀ ≡ (tr→ (Λ w ⇨ A w) δ (fst u₀) , tr→ (Λ w ⇨ B w) δ (snd u₀))
  tr←× : {Δ : Tel} (A B : el Δ → Type) (δ : el (ID Δ)) (u₁ : A (δ ₁) × B (δ ₁)) →
    tr← (Λ w ⇨ A w × B w) δ u₁ ≡ (tr← (Λ w ⇨ A w) δ (fst u₁) , tr← (Λ w ⇨ B w) δ (snd u₁))
...
postulate
  lift→× : {Δ : Tel} (A B : el Δ → Type) (δ : el (ID Δ)) (u₀ : A (δ ₀) × B (δ ₀)) →
    lift→ (Λ w ⇨ A w × B w) δ u₀ ≡ (lift→ (Λ w ⇨ A w) δ (fst u₀) , lift→ (Λ w ⇨ B w) δ (snd u₀))
  lift←× : {Δ : Tel} (A B : el Δ → Type) (δ : el (ID Δ)) (u₁ : A (δ ₁) × B (δ ₁)) →
    lift← (Λ w ⇨ A w × B w) δ u₁ ≡ (lift← (Λ w ⇨ A w) δ (fst u₁) , lift← (Λ w ⇨ B w) δ (snd u₁))
...
postulate
  utr→× : {Δ : Tel} (A B : el Δ → Type) (δ : el (ID Δ))
    ...
    (utr→ (Λ w ⇨ A w) δ (fst u₀) (fst u₁) (fst u₁') (fst u₂) (fst u₂') , utr→ (Λ w ⇨ B w) δ (snd u₀) (snd u₁) (snd u₁') (snd u₂) (snd u₂'))
  utr←× : {Δ : Tel} (A B : el Δ → Type) (δ : el (ID Δ))
    (u₁ : A (δ ₁) × B (δ ₁)) (u₀ u₀' : A (δ ₀) × B (δ ₀))
    (u₂ : Id (Λ w ⇨ A w × B w) δ u₀ u₁) (u₂' : Id (Λ w ⇨ A w × B w) δ u₀' u₁) →
    utr← (Λ w ⇨ A w × B w) δ u₁ u₀ u₀' u₂ u₂' ≡
    ...
    (coe← (Id-AP {ε ▸ (Λ _ ⇨ A (δ ₁) × B (δ ₁))} {ε ▸ (Λ _ ⇨ A (δ ₁))}
              ...
              ([] ∷ u₁ ∷ u₁' ∷
               (utr→ (Λ z ⇨ A z) δ (fst u₀) (fst u₁) (fst u₁') (fst u₂) (fst u₂') ,
                utr→ (Λ w ⇨ B w) δ (snd u₀) (snd u₁) (snd u₁') (snd u₂) (snd u₂')))
              (Λ w ⇨ Id (Λ z ⇨ A z) δ (fst u₀) (top w))
              (fst u₂) (fst u₂'))
           (ulift→ (Λ w ⇨ A w) δ (fst u₀) (fst u₁) (fst u₁') (fst u₂) (fst u₂')) ,
     coe← (Id-AP {ε ▸ (Λ _ ⇨ A (δ ₁) × B (δ ₁))} {ε ▸ (Λ _ ⇨ B (δ ₁))}
              (λ x → [] ∷ snd (top x))
              ([] ∷ u₁ ∷ u₁' ∷
               (utr→ (Λ z ⇨ A z) δ (fst u₀) (fst u₁) (fst u₁') (fst u₂) (fst u₂') ,
                utr→ (Λ w ⇨ B w) δ (snd u₀) (snd u₁) (snd u₁') (snd u₂) (snd u₂')))
              (Λ w ⇨ Id (Λ z ⇨ B z) δ (snd u₀) (top w))
              (snd u₂) (snd u₂'))
           (ulift→ (Λ w ⇨ B w) δ (snd u₀) (snd u₁) (snd u₁') (snd u₂) (snd u₂')))
  ulift←× : {Δ : Tel} (A B : el Δ → Type) (δ : el (ID Δ))
    (u₁ : A (δ ₁) × B (δ ₁)) (u₀ u₀' : A (δ ₀) × B (δ ₀))
    (u₂ : Id (Λ w ⇨ A w × B w) δ u₀ u₁) (u₂' : Id (Λ w ⇨ A w × B w) δ u₀' u₁) →
    ulift← (Λ w ⇨ A w × B w) δ u₁ u₀ u₀' u₂ u₂' ≡
